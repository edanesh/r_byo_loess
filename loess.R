...
train <- function() {
    ...
    if (!is.null(training_params$span)) {
        span <- as.numeric(training_params$span)}
    else {
        span <- 1.0}

#    if (!is.null(training_params$control)) {
#        control <- as.logical(training_params$control)}
#    else {
...
#                             function(x) {levels(x)})
    ...
    loess_model <- loess(formula, training_data,
                   degree=degree, span=span, control=loess.control(surface = "direct"))
    
    # Generate outputs
    #loess_model <- model[!(names(model) %in% c('x', 'residuals', 'fitted.values'))]
    #attributes(loess_model)$class <- 'loess'
    saveRDS(loess_model, file=paste(model_path, 'loess_model.rds', sep='/'))
    print(summary(loess_model))
    #print(paste('gcv:', mars_model$gcv))
    print(paste('mse:', sum((loess_model$fitted.values - training_data[, target]) ** 2)))
